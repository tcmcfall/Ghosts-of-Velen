(function () {
  const PANEL_ID = "compendium-panel";
  const HEADER_SELECTOR = ".panel-header";
  const TITLE_SELECTOR = ".panel-title";

  // ——— Public API ———
  window.openCompendiumPanel = function openCompendiumPanel() {
    ...
    if (!panel) {
      ...
      panel.innerHTML = `
        <div class="panel-header" style="display:flex;align-items:center;justify-content:space-between;user-select:none;cursor:move;">
          <span class="panel-title">Compendium</span>
          <div>
        ...
        <iframe src="panels/compendium-panel.html" style="border:0;width:100%;height:calc(100% - 40px);" loading="lazy" referrerpolicy="no-referrer"></iframe>
      `;
